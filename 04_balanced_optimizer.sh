...
if [ -z "$LATEST_OPT_DIR" ]; then
    echo "❌ Error: No optimization results found in optimization_results/"
    exit 1
fi
...
def main():
    ...
    print("⚖️  Applying balanced scoring...")
    ...
    print(f"✅ Saved recommendations: {output_json}")
    ...
    with open(output_report, 'w') as f:
        ...
        f.write("METHODOLOGY\n")
        f.write("-----------\n")
        f.write("• FA penalty: 70% reduction (extreme effect sizes)\n")
        f.write("• COUNT/NCOUNT2 bonus: +20% (moderate effects)\n")
        f.write("• QA bonus: +10% (balanced anisotropy)\n")
        f.write("• Dominance penalty: Up to 60% for scores >0.9\n\n")
    ...
    print(f"📊 SUMMARY:")
    print(f"   • {summary['total_combinations']} combinations processed")
    print(f"   • {summary['fa_combinations']} FA combinations (penalized)")
    print(f"   • {summary['count_ncount2_combinations']} COUNT/NCOUNT2 combinations (favored)")
    print(f"   • Mean balanced score: {summary['mean_balanced_score']:.3f}")
...
echo ""
echo "🎯 This balanced optimization replaces 03_optimize_metrics.sh"
echo "   It addresses FA dominance while maintaining scientific rigor."
echo ""
echo "📋 UPDATED WORKFLOW:"
echo "   1. ✅ 01_setup.sh"
echo "   2. ✅ 02_compute_graph_metrics.sh"
echo "   3. ❌ 03_optimize_metrics.sh (DEPRECATED)"
echo "   4. ✅ 04_balanced_optimizer.sh (NEW - USE THIS)"
echo ""
echo "🏆 Ready for group comparisons with balanced atlas-metric combinations!"
