...
class ConnectivityExtractor:
    ...
    def create_output_structure(self, output_dir: str, base_name: str) -> Path:
        ...
        track_count = self.config.get('tract_count', self.config.get('track_count', 100000))  # Support both naming conventions
    ...
    def extract_connectivity_matrix(self, input_file: str, output_dir: Path, 
        ...
        # Note: We create a temp track file but delete it afterwards to save space
        temp_track_file = f'{output_prefix}_temp.tt.gz'
        cmd = [
        ...
        try:
            ...
            if success:
                ...
                # Clean up temporary track file to save disk space
                try:
                    if Path(temp_track_file).exists():
                        Path(temp_track_file).unlink()
                        self.logger.debug(f"Deleted temporary track file: {temp_track_file}")
                except Exception as e:
                    self.logger.warning(f"Could not delete temporary track file {temp_track_file}: {e}")
            else:
