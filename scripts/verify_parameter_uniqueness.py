...
def analyze_parameter_uniqueness(matrices_dir):
    ...
    if len(duplicate_matrices) > 0:
        ...
        for dup in duplicate_matrices[:5]:  # Show first 5
            ...
            logging.error(f"Duplicate files: {dup['files'][:2]}...")  # Show first 2 files
    else:
    ...
    if not results_df.empty:
        ...
        low_diversity = results_df[results_df['diversity_score'] < results_df['diversity_score'].quantile(0.1)]
        if not low_diversity.empty:
            logging.warning(f"\n⚠️  Groups with low parameter diversity (bottom 10%):")
            for _, row in low_diversity.iterrows():
                logging.warning(f"  {row['group']}: diversity_score = {row['diversity_score']:.6f}")
    ...
    return {
        ...
        'summary': {
            ...
            'mean_diversity': results_df['diversity_score'].mean() if not results_df.empty else 0
        }
    }
...
def main():
    ...
    parser.add_argument('matrices_dir', help='Directory containing connectivity matrices')
