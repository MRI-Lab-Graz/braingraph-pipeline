...
from scipy.stats import zscore, ttest_ind, mannwhitneyu, wilcoxon, friedmanchisquare
...
class MetricOptimizer:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            "metrics_to_evaluate": [
                "sparsity", "small_worldness", "modularity", 
    ...
    def compute_sparsity_score(self, sparsity_values: np.ndarray) -> np.ndarray:
        ...
        out_of_range = (sparsity_values < min_sparsity) | (sparsity_values > max_sparsity)
    ...
    def compute_reliability_score(self, df: pd.DataFrame, 
    ...
    def optimize_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        high_quality = df_with_scores[df_with_scores['quality_score'] >= self.quality_threshold]
        
        if len(high_quality) == 0:
            logger.warning(f"No combinations meet quality threshold {self.quality_threshold}")
        ...
        df_with_scores['meets_quality_threshold'] = df_with_scores['quality_score'] >= self.quality_threshold
        ...
        logger.info(f"Optimization complete. {len(high_quality)} high-quality combinations found.")
    ...
    def generate_optimization_summary(self, df: pd.DataFrame) -> Dict:
        ...
        # Metric-wise best
        metric_best = df.loc[df.groupby('connectivity_metric')['quality_score'].idxmax()][
            ['atlas', 'connectivity_metric', 'quality_score']
        ]
        summary['best_per_metric'] = metric_best.to_dict('records')
        
        # Quality component analysis
        component_cols = [col for col in df.columns if col.endswith('_score') and col != 'quality_score']
        if component_cols:
            summary['component_correlations'] = {}
            for component in component_cols:
                corr = df[component].corr(df['quality_score'])
                summary['component_correlations'][component] = float(corr) if not np.isnan(corr) else 0.0
...
"""
    ...
    def create_optimization_plots(self, df: pd.DataFrame, output_dir: str) -> List[str]:
        ...
        plt.axvline(self.quality_threshold, color='red', linestyle='--', label=f'Threshold ({self.quality_threshold})')
        ...
        plt.subplot(2, 2, 2)
        atlas_order = df.groupby('atlas')['quality_score'].mean().sort_values(ascending=False).index
        ...
        plt.subplot(2, 2, 3)
        metric_order = df.groupby('connectivity_metric')['quality_score'].mean().sort_values(ascending=False).index
        sns.boxplot(data=df, x='connectivity_metric', y='quality_score', order=metric_order)
        plt.xticks(rotation=45)
        plt.ylabel('Quality Score')
        ...
        if 'sparsity' in df.columns and 'small_worldness' in df.columns:
            ...
            plt.subplot(1, 2, 1)
            scatter = plt.scatter(df['sparsity'], df['small_worldness'], 
            ...
            if 'modularity' in df.columns and 'global_efficiency' in df.columns:
                scatter = plt.scatter(df['modularity'], df['global_efficiency'], 
                                    c=df['quality_score'], cmap='viridis', alpha=0.7)
                plt.colorbar(scatter, label='Quality Score')
    ...
    def _perform_pairwise_comparisons(self, metric_data: Dict[str, np.ndarray], 
                                    target_metric: str, atlas: str) -> Dict:
        ...
        for i in range(len(metric_names)):
            for j in range(i + 1, len(metric_names)):
                ...
                use_parametric = (
                    comparison['normality_p1'] > 0.05 and 
                    comparison['normality_p2'] > 0.05 and
                    comparison['equal_var_p'] > 0.05
                )
    ...
    def _generate_comparison_summary(self, comparisons: List[Dict]) -> Dict:
        ...
        summary = {
            ...
            'significant_uncorrected': len(df_comps[df_comps['p_value'] < 0.05]) if 'p_value' in df_comps.columns else 0,
            'significant_corrected': len(df_comps[df_comps['p_corrected'] < 0.05]) if 'p_corrected' in df_comps.columns else 0,
            'mean_effect_size': df_comps['cohens_d'].abs().mean() if 'cohens_d' in df_comps.columns else 0,
            'large_effects': len(df_comps[df_comps['effect_size'] == 'large']) if 'effect_size' in df_comps.columns else 0
        }
...
def main():
    ...
    try:
        ...
        logger.info(f"Found {df['connectivity_metric'].nunique()} unique connectivity metrics")
    ...
    try:
        ...
        if args.plots:
            plot_files = optimizer.create_optimization_plots(optimized_df, str(output_path))
