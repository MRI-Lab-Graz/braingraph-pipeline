...
def create_bootstrap_configs(data_dir, qa_percentage=0.2, n_waves=2, output_dir="bootstrap_configs"):
    """
    ...
    logging.info(f"📊 Sample size per wave: {n_per_wave} subjects ({n_per_wave/n_total*100:.1f}%)")
    ...
    result_dirs = [f"bootstrap_results_{config['wave_name']}" for config in bootstrap_configs]
    comparison_cmd = f"python bootstrap_qa_validator.py validate {' '.join(result_dirs)}"
...
def compute_bootstrap_qa_metrics(df):
    ...
    quality_measures = ['density', 'global_efficiency(binary)', 'clustering_coeff_average(binary)']
    ...
    for wave in df['bootstrap_wave'].unique():
        ...
        for measure in quality_measures:
            ...
            wave_metrics[f'{measure}_iqr'] = np.percentile(values, 75) - np.percentile(values, 25)
    ...
    for measure in quality_measures:
        ...
        if len(wave_means) >= 2:
            ...
            bootstrap_sem = bootstrap_std / np.sqrt(len(wave_means))  # Standard error of mean
            ...
            cv_between_waves = bootstrap_std / bootstrap_mean if bootstrap_mean != 0 else float('inf')
            ...
            bootstrap_results['bootstrap_confidence'][measure] = {
                ...
                'relative_ci_width': (ci_upper - ci_lower) / bootstrap_mean if bootstrap_mean != 0 else float('inf')
            }
...
def assess_bootstrap_stability(bootstrap_results):
    ...
    for measure, stats_data in bootstrap_results['stability_analysis'].items():
        ...
        if measure in bootstrap_results['bootstrap_confidence']:
            ...
            logging.info(f"   95% CI: [{ci_data['ci_lower']:.4f}, {ci_data['ci_upper']:.4f}]")
    ...
    if stability_scores:
        ...
        if avg_score >= 3.5:
            ...
            recommendation = "✅ QA metrics are highly stable - proceed with full dataset analysis"
        elif avg_score >= 2.5:
            overall_stability = "GOOD"
            recommendation = "✅ QA metrics are stable - suitable for analysis"
        elif avg_score >= 1.5:
            overall_stability = "MODERATE"
            recommendation = "⚠️  Consider increasing QA sample size or additional validation"
        else:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Bootstrap QA Validator with Cross-Validation')
    
    subparsers = parser.add_subparsers(dest='command', help='Available commands')
    
    # Create configs command
    create_parser = subparsers.add_parser('create', help='Create bootstrap validation configurations')
    create_parser.add_argument('data_dir', help='Directory containing .fz files')
    create_parser.add_argument('--qa-percentage', type=float, default=0.2, help='Percentage of data per wave (default: 0.2)')
    create_parser.add_argument('--n-waves', type=int, default=2, help='Number of bootstrap waves (default: 2)')
    create_parser.add_argument('--output-dir', default='bootstrap_configs', help='Output directory for configs')
    
    # Validate command
    validate_parser = subparsers.add_parser('validate', help='Validate bootstrap QA results')
    validate_parser.add_argument('result_dirs', nargs='+', help='Bootstrap result directories')
    validate_parser.add_argument('--output', default='bootstrap_validation_report', help='Output directory')
    
    # Generate command (for parameter optimization workflow)
    generate_parser = subparsers.add_parser('generate', help='Generate bootstrap data from wave configuration')
    generate_parser.add_argument('--wave-config', required=True, help='Wave configuration JSON file')
    ...
    if args.command == 'create':
        ...
        print(json.dumps({
            'overall_assessment': stability_assessment,
            'status': 'completed'
        }))
        ...
        output_directory = wave_config.get('output_directory', f"bootstrap_results_{wave_id}")
        ...
        logging.info(f"🌊 Wave: {wave_name} ({wave_config.get('description', 'No description')})")
        logging.info(f"📁 Data directory: {data_dir}")
        ...
        logging.info(f"📈 Selected {n_subjects} subjects from {len(all_files)} available files")
        
        # Create temporary test configuration for this wave
        temp_test_config = {
            "test_config": {
                "name": wave_id,
                "description": wave_config.get('description', f'Bootstrap wave {wave_name}'),
                "enabled": True
            },
            "data_selection": {
                "source_dir": data_dir,
                ...
                "random_seed": hash(wave_id) % 1000,  # Deterministic but different per wave
                "file_pattern": "*.fz"
            },
            "pipeline_config": {
        ...
        if parameters:
            ...
            temp_test_config["pipeline_config"]["extraction_config"] = wave_extraction_config
            ...
            logging.info(f"   └─ DSI Studio parameters: {[(k,v) for k,v in extraction_config.items() if k in parameter_mapping.values()]}")
        
        # Run pipeline with the temporary configuration
        pipeline_cmd = [
            "python", "run_pipeline.py", 
            "--test-config", temp_config_file,
            "--verbose"
        ]
