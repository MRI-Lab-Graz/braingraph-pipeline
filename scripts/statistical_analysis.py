...
class StatisticalAnalysis:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "metrics_to_analyze": [
                ...
                "modularity", "clustering_coefficient", "assortativity",
                "small_worldness", "sparsity"
            ],
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'timepoint' not in data.columns:
            if 'session' in data.columns:
                ...
                logger.warning("No timepoint/session variable found. Creating dummy timepoint.")
                data['timepoint'] = 1
        ...
        logger.info(f"Prepared data: {len(data)} observations, {data['subject_id'].nunique()} subjects")
        
        return data
    
    def fit_statistical_model(self, data: pd.DataFrame, metric: str, 
        ...
        try:
            ...
            if model_type == "mixed" and "(" in formula and "|" in formula:
                ...
                try:
                    ...
                    if "|" in groups_part:
                        ...
                        model = mixedlm(formula, model_data, groups=model_data[group_var])
            ...
            model_results = {
                ...
                "conf_int": result.conf_int().to_dict() if hasattr(result, 'conf_int') else {},
    ...
    def compute_effect_sizes(self, data: pd.DataFrame, metric: str, 
                           grouping_var: str = 'timepoint') -> Dict[str, float]:
        """
                ...
                if len(groups) >= 2:
                    ...
                    for i, group1 in enumerate(groups):
                        for group2 in groups[i+1:]:
                            group1_data = metric_data[metric_data[grouping_var] == group1][metric]
                            group2_data = metric_data[metric_data[grouping_var] == group2][metric]
                            ...
                            if len(group1_data) > 0 and len(group2_data) > 0:
                                ...
                                if pooled_std > 0:
                                    ...
                                    effect_sizes[f"cohens_d_{group1}_vs_{group2}"] = cohens_d
                
                # Eta-squared (proportion of variance explained)
                if len(groups) > 1:
                    try:
                        from scipy.stats import f_oneway
                        group_data = [metric_data[metric_data[grouping_var] == group][metric].values 
                                     for group in groups if len(metric_data[metric_data[grouping_var] == group]) > 0]
                        
                        if len(group_data) > 1:
                            f_stat, p_val = f_oneway(*group_data)
                            
                            # Calculate eta-squared
                            ss_total = np.sum([(x - np.mean(metric_data[metric]))**2 for x in metric_data[metric]])
                            ss_between = np.sum([len(group) * (np.mean(group) - np.mean(metric_data[metric]))**2 
                        ...
                        logger.warning(f"Could not compute eta-squared for {metric}: {e}")
    ...
    def correct_multiple_comparisons(self, pvalues: List[float], 
                                   method: str = None) -> Tuple[List[bool], List[float]]:
        """
    ...
    def analyze_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        grouping_vars = self.config.get("grouping_variables", ["atlas", "connectivity_metric"])
        ...
        for group_name, group_data in data.groupby(available_grouping):
            ...
            for metric in available_metrics:
                ...
                if "pvalues" in model_results and isinstance(model_results["pvalues"], dict):
        ...
        final_results = {
            ...
            "multiple_comparison_method": self.config.get("multiple_comparison_correction", "bonferroni")
        }
    ...
    def _generate_analysis_summary(self, results: Dict) -> Dict[str, Any]:
        ...
        for group_key, group_results in results.items():
            ...
            for metric, metric_results in group_results.items():
                ...
                if "corrected_pvalues" in metric_results:
                    for param, correction_info in metric_results["corrected_pvalues"].items():
                        total_tests += 1
                ...
                if "effect_sizes" in metric_results:
                    for effect_name, effect_value in metric_results["effect_sizes"].items():
        ...
        summary["overall_statistics"] = {
            ...
            "proportion_significant": significant_count / total_tests if total_tests > 0 else 0,
    ...
    def generate_plots(self, df: pd.DataFrame, output_dir: str) -> List[str]:
        ...
        # Prepare data
        data = self.prepare_data(df)
        metrics_to_plot = [m for m in self.config.get("metrics_to_analyze", []) if m in data.columns]
        ...
        if hasattr(self, 'results') and 'analysis_results' in self.results:
            ...
            if effect_sizes_data:
                ...
                plt.axhline(y=0.5, color='orange', linestyle='--', label='Medium effect')
                plt.axhline(y=0.8, color='green', linestyle='--', label='Large effect')
                plt.legend()
                
                # Plot by group
                plt.subplot(2, 2, 2)
                if len(effect_df['group'].unique()) <= 10:  # Only if not too many groups
                ...
                plt.subplot(2, 2, 3)
                plt.hist(effect_df['abs_effect_size'], bins=30, alpha=0.7, edgecolor='black')
                ...
                plt.yticks(y_pos, [f"{row['metric'][:10]}..." for _, row in top_effects.iterrows()])
                plt.xlabel('Absolute Effect Size')
        ...
        correlation_metrics = [m for m in metrics_to_plot if data[m].notna().sum() > 10][:15]  # Limit to 15 metrics
        ...
        if hasattr(self, 'results') and 'summary' in self.results:
            ...
            if group_summaries:
                ...
                sig_effects = [group_summaries[g].get('significant_effects', 0) for g in groups]
                
                plt.bar(range(len(groups)), sig_effects)
                plt.xticks(range(len(groups)), [g[:15] + "..." if len(g) > 15 else g for g in groups], 
                          rotation=45)
            ...
            plt.subplot(2, 2, 3)
            if group_summaries:
                large_effects = [group_summaries[g].get('large_effects', 0) for g in groups]
                ...
                for group_key, group_results in value.items():
                    ...
                    for metric, metric_results in group_results.items():
                        ...
                        for res_key, res_value in metric_results.items():
                            if res_key == 'model_results' and isinstance(res_value, dict):
                                # Remove model object
                                model_copy = {k: v for k, v in res_value.items() if k != 'model_object'}
                                results_copy[key][group_key][metric][res_key] = model_copy
                            else:
                                results_copy[key][group_key][metric][res_key] = res_value
            else:
    ...
    def _write_summary_report(self, output_file: str) -> None:
        ...
        with open(output_file, 'w') as f:
            ...
            for group, group_summary in group_summaries.items():
                ...
                f.write(f"  Significant effects: {group_summary.get('significant_effects', 0)}\n")
        ...
        for group_key, group_results in self.results['analysis_results'].items():
            for metric, metric_results in group_results.items():
                base_row = {
                    'group': group_key,
                    'metric': metric,
                    'n_obs': metric_results.get('descriptive_stats', {}).get('n_obs', np.nan),
                    'mean': metric_results.get('descriptive_stats', {}).get('mean', np.nan),
                    'std': metric_results.get('descriptive_stats', {}).get('std', np.nan),
                    'median': metric_results.get('descriptive_stats', {}).get('median', np.nan)
                }
                ...
                if 'corrected_pvalues' in metric_results:
                    for param, correction_info in metric_results['corrected_pvalues'].items():
                        base_row[f'corrected_pvalue_{param}'] = correction_info['corrected_pvalue']
                        base_row[f'significant_corrected_{param}'] = correction_info['significant_corrected']
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Statistical analysis of connectivity metrics")
