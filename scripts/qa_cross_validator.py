...
def compute_qa_metrics(df):
    ...
    for measure in quality_measures:
        ...
        qa_summary[f'{measure}_cv'] = np.std(values) / (np.mean(values) + 1e-10)  # Coefficient of variation
    ...
    qa_summary['combinations_per_subject'] = len(df) / n_subjects if n_subjects > 0 else 0
...
def compare_qa_metrics(qa_a, qa_b, set_a_name="Set A", set_b_name="Set B"):
    ...
    for metric in sorted(common_metrics):
        ...
        if isinstance(val_a, (int, float)) and isinstance(val_b, (int, float)):
            ...
            comparison_results[metric] = {
                ...
                'rel_diff': rel_diff,
                'agreement': 'Good' if rel_diff < 0.2 else 'Moderate' if rel_diff < 0.5 else 'Poor'
            }
            
            logging.info(f"📈 {metric:<35}: {val_a:.4f} vs {val_b:.4f} (rel_diff: {rel_diff:.3f})")
    ...
    if avg_rel_diff < 0.15:
        ...
        stability = "POOR"
        recommendation = "❌ QA metrics are unstable - increase sample size or check parameters"
...
def generate_comparison_report(comparison, output_dir):
    ...
    with open(report_file, 'w') as f:
        ...
        for key, value in comparison.items():
            if isinstance(value, dict):
                json_compatible[key] = {k: float(v) if isinstance(v, np.floating) else v 
                                      for k, v in value.items()}
            else:
                json_compatible[key] = float(value) if isinstance(value, np.floating) else value
    ...
    try:
        ...
        colors = ['green' if rd < 0.2 else 'orange' if rd < 0.5 else 'red' for rd in rel_diffs]
        ...
        plt.axvline(x=0.2, color='orange', linestyle='--', alpha=0.7, label='Good threshold')
        plt.axvline(x=0.5, color='red', linestyle='--', alpha=0.7, label='Poor threshold')
        plt.legend()
        plt.tight_layout()
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Compare QA results between two random subsets')
    ...
    # Compare metrics
    comparison = compare_qa_metrics(qa_metrics_a, qa_metrics_b, args.set_a_name, args.set_b_name)
