...
PY_FILES = sorted([p for p in SCRIPTS_DIR.glob('*.py') if p.name != Path(__file__).name and not p.name.startswith('test_')])

DOCSTRING_RE = re.compile(r'^\s*(?:"""|\'\'\')', re.M)
DRYRUN_RE = re.compile(r"--dry-run")
HELP_PRINT_RE = re.compile(r"parser\.print_help\(|len\(sys\.argv\)\s*==\s*1|parser\.print_help\(\)")
...
for p in PY_FILES:
    ...
    if not prints_help:
        ...
        failures.append((p.name, 'Does not print help when run without args (parser.print_help or explicit check)'))
...
if failures:
    ...
    for name, reason in failures:
        print(f' - {name}: {reason}')
    print('\nNext steps:')
    print(' - Add a short top-level docstring or scripts/<script>.md with usage examples')
    print(' - Add a --dry-run flag that performs a safe, no-side-effect run')
    print(' - Ensure running the script without arguments prints help (parser.print_help())')
    sys.exit(1)
