...
def _bootstrap_venv_if_available() -> None:
    ...
    for venv in candidate_venvs:
        ...
        py_path = posix_py if posix_py.exists() else (win_py if win_py.exists() else None)
...
def setup_logging(verbose: bool = False, quiet: bool = False, log_dir: Path | None = None) -> logging.Logger:
...
def phase1_parameter_sweep(config_path: Path, data_dir: Path, output_dir: Path, 
                          subjects: int = 3, quick: bool = False, 
    ...
    if subjects > total_subjects:
        logger.warning(f"⚠️  Requested {subjects} subjects, but only {total_subjects} available")
...
def phase2_full_analysis(config_path: Path, data_dir: Path, output_dir: Path,
                        candidate_index: int = 1, logger: logging.Logger = None) -> bool:
    """
    Phase 2: Full Dataset Analysis
    ...
    cmd = [
        ...
        'analyze',
        '-i', str(data_dir),
        '--optimal-config', str(config_path),
        '-o', str(output_dir),
...
def auto_workflow(config_path: Path, data_dir: Path, output_dir: Path,
                 subjects: int = 3, quick: bool = False, 
                 logger: logging.Logger = None) -> bool:
    """
    ...
    success1 = phase1_parameter_sweep(config_path, data_dir, phase1_output, 
                                     subjects, quick, logger)
    ...
    top_candidates = phase1_output / 'optimize' / 'optimization_results' / 'top3_candidates.json'
    ...
    success2 = phase2_full_analysis(top_candidates, data_dir, phase2_output,
                                   candidate_index=1, logger=logger)
...
For detailed documentation, see README.md
    ...
    subparsers = parser.add_subparsers(dest='command', required=True,
                                      help='OptiConn operation mode')
    
    # Phase 1: Parameter sweep
    sweep_parser = subparsers.add_parser('sweep', help='Phase 1: Parameter sweep & QA')
    sweep_parser.add_argument('--config', '-c', required=True, type=Path,
                             help='JSON configuration file with sweep parameters')
    sweep_parser.add_argument('--data', '-d', required=True, type=Path,
                             help='Directory containing subject .fz/.fib.gz files')
    sweep_parser.add_argument('--output', '-o', type=Path, default=Path('results'),
                             help='Output directory (default: results)')
    sweep_parser.add_argument('--subjects', '-n', type=int, default=3,
                             help='Number of subjects for parameter testing (default: 3)')
    sweep_parser.add_argument('--quick', action='store_true',
                             help='Quick mode: use minimal parameter grid')
    
    # Phase 2: Full analysis
    apply_parser = subparsers.add_parser('apply', help='Phase 2: Apply optimal parameters')
    apply_parser.add_argument('--config', '-c', required=True, type=Path,
                             help='JSON file with optimal parameters (from Phase 1)')
    apply_parser.add_argument('--data', '-d', required=True, type=Path,
                             help='Directory containing subject .fz/.fib.gz files')
    apply_parser.add_argument('--output', '-o', type=Path, default=Path('analysis_results'),
                             help='Output directory (default: analysis_results)')
    apply_parser.add_argument('--candidate', type=int, default=1,
                             help='Candidate index to use (1=best, 2=second, etc.)')
    
    # Automatic workflow
    auto_parser = subparsers.add_parser('auto', help='Automatic: Phase 1 → Phase 2')
    auto_parser.add_argument('--config', '-c', required=True, type=Path,
                            help='JSON configuration file with sweep parameters')
    auto_parser.add_argument('--data', '-d', required=True, type=Path,
                            help='Directory containing subject .fz/.fib.gz files')
    auto_parser.add_argument('--output', '-o', type=Path, default=Path('auto_results'),
                            help='Base output directory (default: auto_results)')
    auto_parser.add_argument('--subjects', '-n', type=int, default=3,
                            help='Number of subjects for Phase 1 testing (default: 3)')
    auto_parser.add_argument('--quick', action='store_true',
    ...
    try:
        if args.command == 'sweep':
            success = phase1_parameter_sweep(
                args.config, args.data, args.output, 
                args.subjects, args.quick, logger
            )
            
            if success:
                logger.info("💡 Next step: Run Phase 2 with the top candidate")
                top_candidates = args.output / 'optimize' / 'optimization_results' / 'top3_candidates.json'
                logger.info(f"   python opticonn_new.py apply --config {top_candidates} --data {args.data}")
                return 0
            else:
                return 1
